...
@pytest.mark.parametrize("conflict", [None, "file", "symlink"])
def test_branch_checks_its_default_output_before_emitting_commands(
    restore_lab: RestoreLab, tmp_path: Path, monkeypatch, conflict: str | None
) -> None:
    monkeypatch.chdir(tmp_path)
    source = Path("child.ndjson")
    before = restore_lab.parent_trace.read_bytes()
    ...
    code, stdout, stderr = _invoke(args)
    assert code == 0, stderr
    branch = shlex.split(json.loads(stdout)["branch_command"])[1:]
    output = Path("child.child.ndjson")
    if conflict:
        if conflict == "symlink":
            output.symlink_to(source)
        else:
            output.write_text("preserved conflicting artifact")
        for command in (args, branch):
            code, stdout, stderr = _invoke(command)
            assert code == 2
            assert "output" in stderr and not stdout
        assert source.read_bytes() == before
        return

    code, stdout, stderr = _invoke(branch)
    assert code == 0, stderr
    ready = json.loads(stdout)
    assert ready["status"] == "ready" and not output.exists()
    assert shlex.split(ready["next_command"])[-2:] == ["-o", str(output)]
    code, stdout, stderr = _invoke(shlex.split(ready["next_command"])[1:])
    assert code == 0, stderr
    executed = json.loads(stdout)
    assert executed["next_command"] == "dynamical validate child.child.ndjson --json"
    assert _invoke(shlex.split(executed["next_command"])[1:])[0] == 0
    assert source.read_bytes() == before
...
def test_validate_names_a_verified_branch_command(restore_lab: RestoreLab, tmp_path: Path) -> None:
    ...
    output = restore_lab.parent_trace.with_name("parent.child.ndjson")
    assert shlex.split(dry_run["next_command"]) == ["dynamical", *expected, "-o", str(output)]

    code, stdout, _ = _invoke(shlex.split(dry_run["next_command"])[1:])
    assert code == 0
