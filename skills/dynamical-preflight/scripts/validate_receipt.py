#!/usr/bin/env python3
# ruff: noqa: E501, I001
"""Freeze one compact, model-authored preflight map."""
...
from dynamical.composition import preflight_state_sha256
from dynamical.schema import canonical_sha256, load_campaign_requirement
from dynamical.schema import load_capability_registry, load_facility_manifest

# The file has a strict size limit. Keep short record builders readable as one line.
# fmt: off
...
def finalize(data: dict[str, Any], mapping_path: Path, args: argparse.Namespace) -> dict[str, Any]:
    allowed = {"created_at_utc", "discovery_roots", "sources", "entities", "facts", "relations", "gaps", "requested_cutoff", "parent"}
    ...
    for item in _records(data, "entities"):
        semantic = {key: value for key, value in item.items() if key not in {"ref", "evidence_refs"}}
        ...
        entities.append({"entity_id": entity_id, **semantic, "evidence_refs": _evidence(item.get("evidence_refs", []), source_ids)})

    raw_facts, fact_ids, semantics = _records(data, "facts"), {}, {}
    ...
    for item in raw_facts:
        ...
        facts.append({"fact_id": fact_ids[item["ref"]], **semantics[item["ref"]], "evidence_refs": _evidence(item.get("evidence_refs", []), source_ids), "input_ids": sorted(fact_ids[ref] for ref in inputs)})
    ...
    state_relations = sorted(item["relation_id"] for item in relations if item.get("state_defining") is True)
    ...
    receipt = {
        "document_type": "dynamical.preflight-receipt", "schema_version": "0.1.0",
        "created_at_utc": created, "status": "HOLD" if material_gaps or assumed else "READY",
        "discovery_roots": sorted(source_ids[ref] for ref in root_refs), "sources": sources,
        "entities": sorted(entities, key=lambda item: item["entity_id"]), "facts": sorted(facts, key=lambda item: item["fact_id"]),
        "relations": sorted(relations, key=lambda item: item["relation_id"]), "gaps": sorted(gaps, key=lambda item: item["gap_id"]),
        "state": {"fact_ids": state_facts, "relation_ids": state_relations, "evidence_cutoff": cutoff, "parent": data.get("parent")},
        "handoff": {name: {"path": str(getattr(args, name).resolve()), "canonical_sha256": canonical_sha256(document.model_dump(mode="json"))} for name, document in documents.items()},
        "authority": {"provider_admission_changed": False, "physical_authority_granted": False, "qualification_decision_made": False},
    }
...
def _self_test() -> None:
    receipt = {"facts": [{"fact_id": "f", "subject_id": "e", "field": "mass", "value": 1, "kind": "observation", "available_at": "2026-01-01T00:00:00Z", "state_path": "/sample/mass", "evidence_refs": [], "input_ids": []}], "relations": [], "state": {"fact_ids": ["f"], "relation_ids": [], "evidence_cutoff": "2026-01-01T00:00:00Z", "parent": None}}
    first = preflight_state_sha256(receipt)
    receipt["facts"][0]["value"] = 2
    assert preflight_state_sha256(receipt) != first
    print('{"check":"preflight-finalizer","status":"passed"}')

def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("mapping", nargs="?", type=Path)
    for name in ("requirement", "registry", "facility", "output"):
    ...
    if args.self_test:
        _self_test()
        return 0
    if not all((args.mapping, args.requirement, args.registry, args.facility, args.output)):
        parser.error("mapping and all four path flags are required")
    mapping = json.loads(args.mapping.read_text(encoding="utf-8"))
    if not isinstance(mapping, dict):
        raise ValueError("mapping must contain one JSON object")
    receipt = finalize(mapping, args.mapping, args)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(receipt, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps({"status": receipt["status"], "state_id": receipt["state"]["state_id"], "state_sha256": receipt["state"]["state_sha256"], "output": str(args.output)}, sort_keys=True, separators=(",", ":")))
    return 0
